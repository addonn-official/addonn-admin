...
export const useDeleteSubTopic = (topic_id?: string) => {
	...
	return useMutation({
		mutationFn: ecommerceApi.deleteSubTopic,
		onSuccess: (_data, variables) => {
			// Prefer the course_id passed as mutation variable, fallback to hook arg.
            console.log(variables)
			const cid = (variables && (variables as any).topic_id) || topic_id;
			if (cid) {
				...
				queryClient.invalidateQueries({
					predicate: (query) =>
						...
						query.queryKey.some((k) => typeof k === 'string' && k.toLowerCase().includes('topic')),
				});
			}
		},
